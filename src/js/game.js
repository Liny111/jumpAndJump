var Game = function () {
  ...
  var planceGeometry = new THREE.PlaneGeometry(
    this.size.width,
    this.size.height
  ) // PlaneGeometry: 翻译 平面几何    (参数: 宽60, 高20)
  var planeMaterial = new THREE.MeshLambertMaterial({ color: 0x898898 }) // MeshLambertMaterial: 翻译 网格材质    (用来设置平面的外观, 颜色，透明度等)
...
Game.prototype = {
  ...
  _handleMousedown: function () {
    ...
    function act() {
      ...
      // 以jumperBody蓄力一半为最大值
      if (
        !self.jumperStat.ready &&
        self.jumperBody.scale.y > 0.02 &&
        self.jumperBody.scale.y >= 0.5
      ) {
  ...
  _handleMouseup: function () {
    ...
    function act() {
      ...
      if (self.jumperStat.ySpeed > 0 || self.jumper.position.y >= 3) {
        // jumper根据下一个方块的位置来确定水平运动方向

        var jumperRotateBase = self.jumperStat.yTimes / 10
  ...
  _createJumper: function () {
    ...
    var bodyGeometry = new THREE.CylinderGeometry(
      this.config.jumperWidth / 3,
      this.config.jumperDeep / 2,
  ...
  _createCube: function () {
    ...
    if (this.cubes.length) {
      ...
      if (this.cubeStat.nextDir === 'left') {
        mesh.position.x =
          this.cubes[this.cubes.length - 1].position.x - 4 * Math.random() - 6
      } else {
        mesh.position.z =
          this.cubes[this.cubes.length - 1].position.z - 4 * Math.random() - 6
      }
    }
    console.log(1)
    this.cubes.push(mesh)
    // 当方块数大于9时，删除前面的方块，因为不会出现在画布中
  ...
  _createMaterial: function () {
    ...
    function Chess() {
      ...
      //   'https://raw.githubusercontent.com/Ovilia/ThreeExample.js/master/img/chess.png'
      // )
      var texture = new THREE.TextureLoader().load('./src/image/Rectangle.png')
      var texture1 = new THREE.TextureLoader().load('./src/image/door.jpg')
      texture.wrapS = texture.wrapT = THREE.RepeatWrapping // 指定重复方向为两个方向
      texture.repeat.set(1, 1) // 设置重复次数都为4
  ...
  _createGeometry: function () {
    ...
    if (Math.random() > 0.5) {
      // 添加圆柱型方块
      obj.geometry = new THREE.CylinderGeometry(
      ...
      // 方块
      obj.geometry = new THREE.CubeGeometry(
